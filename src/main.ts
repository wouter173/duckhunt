...
import { Config, Duration, Effect, Layer, Option, PubSub, Redacted, Result, Stream } from "effect";
...
import type { Common } from "mxfx/api/schema";
...
function isRoomMessageEvent(
  event: typeof Common.ClientEventWithoutRoomId.Encoded,
): event is typeof Common.RoomMessageEvent.Encoded {
...
const program = Effect.gen(function* () {
  ...
  yield* Effect.logDebug(
    `Starting mxfx client with user: ${matrixUserName} on server: ${yield* Config.string("MATRIX_HOME_SERVER")}`,
  );
  ...
  yield* Stream.fromPubSub(syncHub).pipe(
    ...
    Stream.filterMap(([roomId, roomData]) =>
      Result.fromNullishOr(roomData.timeline?.events, () => "No timeline events").pipe(
        Result.map((events) => ({ roomId, events })),
      ),
    ),
    Stream.map(({ roomId, events }) => ({
      roomId,
      events: events.filter((event) => event.type === "m.room.message"),
    })),
    Stream.flatMap(({ roomId, events }) =>
      Stream.fromArray(events).pipe(Stream.map((event) => ({ roomId, event }))),
    ),
    Stream.filterMap(({ roomId, event }) =>
      Result.liftPredicate(
        isRoomMessageEvent,
        () => "not room message",
      )(event).pipe(Result.map((event) => ({ roomId, event }))),
    ),
    Stream.runForEach(
      Effect.fn("handleRoomMessage")(function* ({ roomId, event }) {
        yield* Effect.log(
          `Received message ${event.content.body} in room ${roomId}: ${JSON.stringify(event)}`,
        );
...
const mxfxLive = MatrixApi.layer.pipe(
  ...
  Layer.provideMerge(
    InMemoryVault.layerConfig({ values: { accessToken: Config.string("MATRIX_ACCESS_TOKEN") } }),
  ),
